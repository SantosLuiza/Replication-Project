...
library(ggplot2)
library(psych)
B_data <- read_csv("PilotB_data.csv")
head(B_data)
```
...
B_data_small <- filter(B_data, Variable != "hypo") #filtering out hypocrite measures 
    
B_data_condition <- B_data_small %>% 
  select(`Random ID`, Condition, Variable, Evaluation)

Sub_means <- B_data_condition %>%
...
se=sd/sqrt(length(Sub_means$`Random ID`))
ci= se*ciMult

plot <- ggplot(Grand_means, aes(x = Condition, y = grand_mean_evaluation)) + 
   geom_bar(stat="identity") + ylab("Evaluation of Targets") + geom_errorbar(aes(ymin=grand_mean_evaluation-ci, ymax=grand_mean_evaluation+ci), width = .2) + theme_minimal()
...
B_data_hypo <- filter(B_data, Variable == "hypo")
    
B_data_hypo <- B_data_hypo %>% 
  select(`Random ID`, Condition, Variable, Evaluation)

Sub_means_hypo <- B_data_hypo %>%
  group_by(`Random ID`, Condition) %>%
  summarise(sub_mean = mean(Evaluation, na.rm = TRUE))
...
ciMult <- qt(0.95/2 + .5, 1)
sd=sd(Grand_means_hypo$grand_mean_evaluation)
se=sd/sqrt(length(Sub_means_hypo$`Random ID`))
ci= se*ciMult

###Plot
plot_hypo <- ggplot(Grand_means_hypo, aes(x = Condition, y = grand_mean_evaluation)) + 
   geom_bar(stat="identity") + ylab("Hypocrisy Ratings") + geom_errorbar(aes(ymin=grand_mean_evaluation-ci, ymax=grand_mean_evaluation+ci), width = .2) + theme_minimal()
...
B_data_scenario <- B_data_small %>% 
  select(`Random ID`, Condition, Scenario, Variable, Evaluation)

Sub_means_scenario <- B_data_scenario %>%
...
ciMult <- qt(0.95/2 + .5, 1)
sd=sd(Grand_means_scenario$grand_mean_evaluation)
se=sd/sqrt(length(Sub_means_scenario$`Random ID`))
ci= se*ciMult
