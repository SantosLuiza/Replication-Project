...
B_data_small <- B_data_small %>% 
  select(`Random ID`, Condition, Variable, Evaluation)

Sub_means <- B_data_small %>%
...
plot <- ggplot(Grand_means, aes(x = Condition, y = grand_mean_evaluation)) + 
   geom_bar(stat="identity") + ylab("Evaluation of Targets") + geom_errorbar(aes(ymin=grand_mean_evaluation-sd, ymax=grand_mean_evaluation+sd), width = .2) + theme_minimal()
...
Exploratory analysis of hypocrisy ratings across conditions.
...
plot_hypo <- ggplot(Grand_means_hypo, aes(x = Condition, y = grand_mean_evaluation)) + 
   geom_bar(stat="identity") + ylab("Hypocrisy Ratings") + geom_errorbar(aes(ymin=grand_mean_evaluation-sd, ymax=grand_mean_evaluation+sd), width = .2) + theme_minimal()
